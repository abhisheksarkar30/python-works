...
def copy_logic(base_path, target_path, destination, x):
    ...
    if not path.exists(temp_target_path):
        ...
        if path.isdir(temp_base_path):
            create_dir(temp_destination)
            helper(temp_base_path, temp_target_path, temp_destination)
        elif path.isfile(temp_base_path):
            if path.getmtime(temp_base_path) > path.getmtime(temp_target_path) or path.getctime(temp_base_path) > \
                    path.getctime(temp_target_path):
...
def helper(base_path, target_path, destination):
    processed_dirs = set()
    for x in os.listdir(base_path):
        print("Currently processing: " + x)
        copy_logic(base_path, target_path, destination, x)
        processed_dirs.add(x)

    for x in os.listdir(target_path):
        if not processed_dirs.__contains__(x):
            print("Currently processing: " + x)
            copy_logic(target_path, base_path, destination, x)
            processed_dirs.add(x)
        else:
...
def main():
    ...
    helper(base_path, target_path, destination)
